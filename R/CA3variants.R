...
productfigjk <- fiStandard %*% t(gjk)
  ...
  dimnames(gjkStandard) <- list(labelgjk, paste("ax", 1:nc2, sep = ""))
    ...
    inertiaRSS <- (inertia-sum(S$g^2))*n
 inertiapc1coltub <- apply(inertiapc0,1,sum)  
inertiapc2row <- c(apply(inertiapc0,c(2,3),sum))  
